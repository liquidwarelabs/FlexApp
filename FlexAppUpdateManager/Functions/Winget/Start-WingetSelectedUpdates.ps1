# File: Functions\Winget\Start-WingetSelectedUpdates.ps1
# ================================
...
function Start-WingetSelectedUpdates {
    ...
    try {
        ...
        foreach ($row in $updatesGrid.Rows) {
            if ($row.Cells["WingetSelected"].Value -eq $true) {
        ...
        if ($result -eq "Yes") {
            ...
            Update-WingetStatus -Message "Processing $($selectedPackages.Count) selected updates..." -Level Info
            ...
            Set-WingetButtonStates -Processing $true
            Start-PackageUpdate -UpdatePackages $updatePackages -DefaultFile $defaultFile -Server $server -SourceTab "Winget"
        }
    }
    catch {
        Write-LogMessage "Update process failed: $($_.Exception.Message)" -Level Error -Tab "Winget"
        Update-WingetStatus -Message "Update process failed: $($_.Exception.Message)" -Level Error
